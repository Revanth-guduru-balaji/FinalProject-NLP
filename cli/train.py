...
def parse_args():
    ...
    if f"source_tokenizer" not in os.listdir(args.output_dir):
        raise ValueError(f"The source tokenizer is not found in {args.output_dir}")
    
    if f"target_tokenizer" not in os.listdir(args.output_dir):
...
def preprocess_function(
    ...
    # The loop creates tags like adding bos for prefix of target and eos as sufix for target
    # length is -1 because we see one token ahead
    # END OF YOUR ANSWER
    decoder_input_ids = []
    labels = []
    ...
    *,
    ...
    for batch in tqdm(dataloader, desc="Evaluation"):
        with torch.inference_mode():
            ...
            # YOUR ANSWER HERE (please limit your answer to 1-2 sentences):
            # model.forward() :- It is like init method which invokes when we create model object
            # model.generate() :- Generates sequences for models with a LM head. The method currently supports greedy decoding, beam-search decoding, sampling with temperature, sampling with top-k or nucleus sampling.
            # decoder_input_ids it should be in .forward
...
def main():
    ...
    src_tokenizer_path = os.path.join(args.output_dir, f"source_tokenizer")
    tgt_tokenizer_path = os.path.join(args.output_dir, f"target_tokenizer")
    ...
    # YOUR CODE STARTS HERE
    model = TransfomerEncoderDecoderModel(num_layers=args.num_layers, hidden=args.hidden_size,num_heads= args.num_heads, \
    fcn_hidden= args.fcn_hidden, max_seq_len=args.max_seq_length, \
    src_vocab_size=source_tokenizer.vocab_size,tgt_vocab_size=target_tokenizer.vocab_size, \
