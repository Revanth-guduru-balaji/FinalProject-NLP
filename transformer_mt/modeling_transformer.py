...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, decoder_hidden_states, encoder_hidden_states, key_padding_mask=None):
        ...
        x = self.self_attention(decoder_hidden_states, key_padding_mask=None)
        x = self.self_att_layer_norm(x + residual)

        residual = x
        x = self.cross_attention(x,encoder_hidden_states, key_padding_mask=key_padding_mask)
...
class TransfomerEncoderDecoderModel(nn.Module):
    ...
    def _encode(self, input_ids, key_padding_mask):
        ...
        for i,j in enumerate(self.encoder_layers):
            encoder_hidden_states = self.encoder_layers[i](x=x,key_padding_mask=key_padding_mask)
        # YOUR CODE ENDS HERE
    ...
    def _decode(self, encoder_hidden_states, decoder_input_ids, key_padding_mask):
        ...
        x = self._add_positions(x)
        for i,j in enumerate(self.decoder_layers):
            x = self.decoder_layers[i](decoder_hidden_states=x, encoder_hidden_states=encoder_hidden_states, key_padding_mask=key_padding_mask)
