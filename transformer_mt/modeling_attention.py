...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, kv=None, key_padding_mask=None, return_attention=False):
        ...
        if kv is not None:
            k,v,q = self.k(kv),self.v(kv),self.q(q)
        else:
            k,v,q = self.k(q),self.v(q),self.q(q)
        # YOUR CODE ENDS HERE
        ...
        if key_padding_mask is not None:
            ...
            scores.masked_fill_(key_padding_mask.unsqueeze(1).unsqueeze(2).bool(),float('-inf'))
            scores = scores.view(bs * self.num_heads,attending_seq,attended_seq)
            # # key_padding_mask = key_padding_mask.view(bs, 1, 1, attended_seq).   \
            # # expand(-1, self.num_heads, -1, -1).reshape(bs * self.num_heads, 1, attended_seq)
